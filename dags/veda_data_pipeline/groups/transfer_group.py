...
def cogify_choice(dag_run=None):
    ...
    if payload.get("cogify") in ["true", True]:
...
def cogify_copy_task(dag_run=None):
    ...
    return cogify_transfer_handler(
        event_src=config, external_role_arn=external_role_arn
    )


@task
def transfer_data_task(dag_run=None, payload=None):
    ...
    # payload will generally have the same values expected by discovery, so some renames
    # are needed when combining the dicts
    if payload is None:
        payload = {}
    config = {
        **payload,
...
def transfer_data(dag_run=None, payload=None):
    """Transfer data from one S3 bucket to another; s3 copy, no need for docker"""
...
def subdag_transfer():
    with TaskGroup(**group_kwgs) as discover_grp:
        ...
        cogify_and_copy_data = PythonOperator(
            task_id="cogify_and_copy_data",
            trigger_rule="none_failed",
            python_callable=cogify_copy_task,
        )

        (cogify_branching >> [copy_data, cogify_and_copy_data])
        return discover_grp
