...
class VedaWMTS2STACConfig:
    ...
    # TODO: if this is later generated by some library
    # or is stored in S3, get it from there.
    collection_config: WMTS2STACConfig
    schedule: str | None = None

    # gibs url that has <ows:Identifier> for the indexed
    # STAC collection as well as the associated time dimension.
    gibs_url: str | None = None
...
def is_data_current(date: str = "") -> bool:
    ...
    :param date:
        Date string in the format "%Y-%m-%d" representing the latest NRT data date.
    :type date:
        str, optional
    :return:
        True if the NRT data is up-to-date (date is today or a future date),
        False otherwise.
    :rtype: bool
    """
    if not date:
...
def update_nrt_collection_task(previous_collection=None, latest_nrt_date=None):
    """
    Updates the temporal extent of a collection with the latest
    NRT (Near Real-Time) data date.

    This task takes the previous collection metadata and the latest available date
    for NRT data, and updates the collection's temporal extent to include this new data.

    :param previous_collection: The previous collection config as a dictionary.
    :type previous_collection: dict, optional
    :param latest_nrt_date: The latest date for which NRT data is available,
        in 'YYYY-MM-DD' format. Defaults to None.
    :type latest_nrt_date: str, optional
    :return: The updated collection config with the temporal extent updated to include
        the latest NRT data date.
    :rtype: dict or None
    """
    import copy
...
def fetch_metadata_from_gibs(
    ...
    Raises:
        requests.exceptions.RequestException: HTTP request to the GIBS server fails.
    """
    try:
...
@task
def extract_latest_nrt_date_for_collection(
    xml_string: str, collection_id: str = "VIIRS_SNPP_DayNightBand_At_Sensor_Radiance"
) -> str:
    """
    Extracts the latest date from the WMTS GetCapabilities XML response
    for a specified layer.

    This function parses an XML string to find the most recent date available for a
    given collection ID within the WMTS GetCapabilities response. It uses the XML
    namespace to correctly locate the relevant elements and attributes.

    :param xml_string: A string containing the XML response from the
        WMTS GetCapabilities request.
    :type xml_string: str
    :param collection_id: The identifier of the layer for which to extract
        the latest date. Defaults to 'VIIRS_SNPP_DayNightBand_At_Sensor_Radiance'.
    :type collection_id: str, optional
    :return: The latest date as a string, extracted from the XML.
        Returns an empty string if the XML is empty,
    ...
    for layer in layers:
        ...
        if layer_id == collection_id:
            ...
            if dimension_id == "Time":
                return dimension.find("xmlns:Default", XML_NAMESPACE).text
    return ""
...
def validate_collection_task(collection_config: dict) -> dict:
    ...
    if (
        ...
        in collection_config.get("stac_extensions", [])
    ):
...
@task_group(
    group_id="validation_task_group",
    tooltip="validate if collection update is needed via metadata available on gibs",
)
def validation_task_group(gibs_url: str, collection_id: str) -> dict:
    ...
    This group fetches metadata from GIBS which is in XML format, extracts the latest
    date, and checks if an update is needed.

    :param gibs_url: The URL to the GIBS metadata endpoint.
    :param collection_id: The id of the collection in the GIBS.
    :return: A dictionary containing a boolean indicating if an update is needed and
        the latest layer date.
    """
    xml_string_data = fetch_metadata_from_gibs(gibs_url)
    latest_layer_date = extract_latest_nrt_date_for_collection(
        xml_string_data, collection_id
    )
    # if its current, the latest nrt data source is updated to todays date,
    # so need to update in STAC too.
    is_data_current_res = is_data_current(latest_layer_date)
    return {
...
@task_group(
    group_id="collection_update_task_group",
    tooltip="update the nightlight nrt collection with the available nrt date",
)
...
@task_group(
    group_id="gibs_wmts2stac_update_task_group", tooltip="gibs wmts to stac update"
)
def gibs_wmts2stac_update_task_group(
    collection: dict, collection_id: str, gibs_url: str
) -> None:
    """
    Task group to manage the update of the WMTS2STAC sourced from GIBS.

    This task group consists of tasks that:
    1. Validate if an update is needed by comparing the latest available data on GIBS
        with the current collection.
    2. If an update is needed, it updates the NRT collection with
        the latest available date.
    3. Finally, it ingests the updated collection.

    The sub-tasks are grouped into validation and collection update task groups for
    better organization and readability.
    A branching task determines whether to proceed with the update based on the
    validation result.
    ...
    def branch_update_needed(update_needed: bool) -> str:
        ...
        return (
            "gibs_wmts2stac_update_task_group.collection_update_task_group"
            ".update_nrt_collection_task"
        )
    ...
    branch_choice_instance >> [
        no_action_needed,
        collection_update_task_group(collection, latest_layer_date),
    ]
...
VIIRS_SNPP_NRT_collection: WMTS2STACConfig = {
    ...
    "description": (
        "The Black Marble Nighttime At Sensor Radiance (Day/Night Band) layer is "
        "created from NASA's Black Marble daily at-sensor top-of-atmosphere nighttime "
        "radiance product (VNP46A1). It is displayed as a grayscale image. The layer "
        "is expressed in radiance units (nW/(cm2 sr)) with log10 conversion. It is "
        "stretched up to 38 nW/(cm2 sr) resulting in improvements in capturing city "
        "lights in greater spatial detail than traditional Nighttime Imagery resampled "
        "at 0-255 (e.g., Day/Night Band, Enhanced Near Constant Contrast).The ultra-"
        "sensitivity of the VIIRS Day/Night Band enables scientists to capture the "
        "Earth's surface and atmosphere in low light conditions, allowing for better "
        "monitoring of nighttime phenomena. These images are also useful for assessing "
        "anthropogenic sources of light emissions under varying illumination "
        "conditions. For instance, during partial to full moon conditions, the layer "
        "can identify the location and features of clouds and other natural "
        "terrestrial features such as sea ice and snow cover, while enabling temporal "
        "observations in urban regions, regardless of moonlit conditions. As such, the "
        "layer is particularly useful for detecting city lights, lightning, auroras, "
        "fires, gas flares, and fishing fleets.The Black Marble Nighttime At Sensor "
        "Radiance (Day/Night Band) layer is available in near real-time from the "
        "Visible Infrared Imaging Radiometer Suite (VIIRS) aboard the joint NASA/NOAA "
        "Suomi National Polar orbiting Partnership (Suomi NPP) satellite. The sensor "
        "resolution is 750 m at nadir, imagery resolution is 500 m, and the temporal "
        "resolution is daily."
    ),
    "extent": {
        "spatial": {"bbox": [[-180, -90, 180, 90]]},
        "temporal": {"interval": [["2020-11-10T00:00:00Z", "2025-04-14T00:00:00Z"]]},
    },
    "is_periodic": True,
    "item_assets": {
        "cog_default": {
