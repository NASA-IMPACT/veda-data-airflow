...
def handler(event: Dict[str, Any], role_arn: [str, None] = None) -> Union[S3LinkOutput, StacItemOutput]:
    """
    ...
    try:
        stac_item = stac.generate_stac(parsed_event, role_arn=role_arn).to_dict()
...
def sequential_processing(objects, role_arn=None):
    returned_results = []
    for _object in objects:
        result = handler(_object, role_arn)
...
def build_stac_handler(payload_event: dict, bucket_output: str, role_arn: [str, None] = None):
    payload_event_copy = payload_event.copy()
    s3_event = payload_event_copy.pop("payload")
    collection = payload_event_copy.get("collection", "not_provided")
    ...
    payloads = sequential_processing(objects, role_arn=role_arn)
