...
def get_sts_session(role_arn=None):
    if role_arn:
...
def generate_stac(event: events.RegexEvent, role_arn: [str, None] = None) -> pystac.Item:
    """
    ...
    rasterio_kwargs = {}
    rasterio_kwargs["session"] = get_sts_session(role_arn=role_arn)
    with rasterio.Env(
