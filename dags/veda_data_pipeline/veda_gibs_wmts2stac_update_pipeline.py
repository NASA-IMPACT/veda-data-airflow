...
GibsWMTS2STACConfig = dict[str, any] # this mostly comply with a STAC json config

@dataclass
class VedaGibsWMTSConfig:
    id: str
    dag: str
    collection_id: str  # Collection_id used in STAC, needed to find which collection to update.
    gibs_url: str   # gibs url that has <ows:Identifier> and the associated time dimension.
    collection_config: GibsWMTS2STACConfig #TODO: if this is later generated by some library or is stored in S3, get it from there

def get_ingest_gibswmts2stac_dag(id: str, event: VedaGibsWMTSConfig):
    """
    ...
    dag_doc_md = f"""
        ## This DAG handles creation of STAC from (GIBS) WMTS and set a scheduler to check and update the STAC.
        ### How does it update:
        For the frequency set by schedule in VedaGibsWMTSConfig, the DAG checks if the NRT data hosted by earthdata
        is avaialble for the latest available date via. {gibs_url}
        If available, it overrides the {collection_id} collection
        with the updated temporal extent into the STAC.
        #### Note
    ...
    veda_gibs_wmts2stac_with_update(collection_config=collection_config, collection_id=collection_id, gibs_url=gibs_url)
...
get_ingest_gibswmts2stac_dag_config: VedaGibsWMTSConfig = {
