...
This DAG uses the same input as `veda-dataset-pipeline` but adds the ability to transfer
assets to the production bucket. This will mutate the payload, so that item references
will target the new asset locations.
#### Notes
...
template_dag_run_conf = {
    ...
    "transfer": Param(
        True,
        type="boolean",
        description="Transfer assets to production bucket if true (true by default)",
    ),
}


@task(max_active_tis_per_dag=3)
def transfer_assets_to_production_bucket(dag_run=None, payload=None):
    # merge collection id into payload, then transfer data
    if payload is None:
        payload = {}
    payload["collection"] = dag_run.conf.get("collection")
    transfer = payload.get("transfer", dag_run.conf.get("transfer", True))

    config = {
        **payload,
        "origin_bucket": payload.get(
            "bucket", dag_run.conf.get("origin_bucket", "veda-data-store")
        ),
        "origin_prefix": payload.get(
            "prefix", dag_run.conf.get("origin_prefix", "s3-prefix/")
        ),
        "target_bucket": payload.get(
            "target_bucket", dag_run.conf.get("target_bucket", "veda-data-store")
        ),
        "dry_run": payload.get("dry_run", dag_run.conf.get("dry_run", False)),
    }

    if not transfer:
        print("Transfer is disabled. Skipping transfer.")
        return payload
...
with DAG(
    "veda_promotion_pipeline", params=template_dag_run_conf, schedule=None, **dag_args
) as dag:
    ...
    transfer_task = transfer_assets_to_production_bucket.expand(
        payload=extract_from_payload
    )
    discover = discover_from_s3_task.partial(payload=mutate_payload_task).expand(
        event=transfer_task
    )
    collection_grp >> discover  # do not discover until collection exists

    get_files = get_dataset_files_to_process(
        payload=discover
    )  # untangle mapped data format to get iterable payloads from discover step
