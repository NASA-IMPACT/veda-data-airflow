...
from veda_data_pipeline.veda_promotion_pipeline import (
    transfer_assets_to_production_bucket,
)
...
def test_transfer_assets_to_production_bucket_transfer_false(
    mock_task_instance, mock_variable_get, s3
):
...
def test_transfer_assets_to_production_bucket_transfer_true(
    mock_task_instance, mock_variable_get, s3
):
    ...
    with patch("airflow.sdk.Variable.get", side_effect=mock_variable_get):
        ...
        assert all(
            obj["Key"].startswith("test-collection/") for obj in response["Contents"]
        )
        assert result["bucket"] == "veda-data-store"
        assert result["prefix"] == "test-collection/"


def test_transfer_assets_to_production_bucket_412_error(
    mock_task_instance, mock_variable_get, s3
):
    """
    Test that when a file already exists with the same ETag (412 error),
    no error is raised
    """
    mock_task_instance.dag_run.conf["transfer"] = True
